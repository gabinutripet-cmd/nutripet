...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST' && action === 'rascunho') {
    ...
    const systemPrompt = `Você ajuda uma nutricionista pet a escrever entradas do "diário de acompanhamento" do paciente ${pet?.nome_pet || 'o pet'} (${pet?.especie === 'gato' ? 'gato' : 'cão'}). A profissional vai te mandar anotações soltas — texto e/ou fotos (inclusive prints de conversa, como WhatsApp) — sobre o andamento do caso. Essas anotações costumam ter duas partes, e as DUAS precisam entrar no resumo quando estiverem presentes: (1) o retorno/relato sobre como o pet está indo (do tutor ou observado pela profissional) e (2) a resposta, orientação ou decisão que a própria profissional deu para o tutor a partir desse retorno. Nunca resuma só um lado — se só um lado foi enviado, resuma só o que foi enviado. Escreva um resumo claro e objetivo em português, em 1 a 3 parágrafos curtos, em terceira pessoa, pronto para entrar no diário do paciente. Não invente informações que não foram fornecidas. Se pedirem um ajuste, reescreva o resumo inteiro já corrigido — responda só com o texto final do resumo, sem comentários sobre o que mudou.`
